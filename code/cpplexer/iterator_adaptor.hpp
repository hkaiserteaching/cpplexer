...
namespace hpx {
    ...
    namespace detail {
        ...
        struct iterator_adaptor_base
        {
            ...
            using value_type = std::conditional_t<std::is_void_v<Value>,
                lazy_conditional_t<std::is_void_v<Reference>,
            ...
            using reference_type = std::conditional_t<std::is_void_v<Reference>,
                lazy_conditional_t<std::is_void_v<Value>,
                    reference_iterator_traits_helper<Base>,
                    std::add_lvalue_reference<Value>>,
                Reference>;

            using iterator_category =
                lazy_conditional_t<std::is_void_v<Category>,
                    category_iterator_traits_helper<Base>,
                    hpx::type_identity<Category>>;

            using difference_type =
                lazy_conditional_t<std::is_void_v<Difference>,
                    difference_type_iterator_traits_helper<Base>,
                    hpx::type_identity<Difference>>;

            using type = hpx::iterator_facade<Derived, value_type,
                iterator_category, reference_type, difference_type, Pointer>;
        };
    }    // namespace detail
    ...
    class iterator_adaptor
      : public hpx::detail::iterator_adaptor_base<Derived, Base, Value,
            Category, Reference, Difference, Pointer>::type
    {
    protected:
        using base_adaptor_type =
            typename hpx::detail::iterator_adaptor_base<Derived, Base, Value,
                Category, Reference, Difference, Pointer>::type;

        friend class hpx::iterator_core_access;

    public:
        iterator_adaptor() = default;

        explicit constexpr iterator_adaptor(Base const& iter) noexcept
          : iterator_(iter)
        {
        }
    ...
    protected:
        ...
        constexpr Base const& base_reference() const noexcept
        {
            return iterator_;
        }

        Base& base_reference() noexcept
        {
            return iterator_;
        }
    ...
    private:
        ...
        constexpr typename base_adaptor_type::reference dereference() const
        ...
        void advance(DifferenceType n) noexcept(noexcept(std::advance(
            std::declval<Base&>(), std::declval<DifferenceType>())))
        {
            std::advance(iterator_, n);
        }

        void increment() noexcept(noexcept(++std::declval<Base&>()))
        {
            ++iterator_;
        }
        ...
        void decrement() noexcept(noexcept(--std::declval<Base&>()))
        {
            --iterator_;
        }
        ...
        template <typename OtherDerived, typename OtherIterator, typename V,
            typename C, typename R, typename D, typename P>
        constexpr typename base_adaptor_type::difference_type distance_to(
            iterator_adaptor<OtherDerived, OtherIterator, V, C, R, D, P> const&
                y) const noexcept(noexcept(std::distance(std::declval<Base>(),
            std::declval<OtherIterator>())))
        {
